...
import {
  upsertWatchChannel,
  markWatchChannelStopped,
  getExpiringWatchChannels,
  getClientsWithOAuth,
  type WatchChannel,
} from './supabase.js';
...
export async function createWatchChannel(
  ...
  try {
    ...
    console.log(
      `Created watch channel ${channelId} for client ${clientId}, calendar ${calendarId}, expires ${expiration.toISOString()}`
    );
 ...
 */
async function stopOldChannel(auth: OAuth2Client, channel: WatchChannel): Promise<void> {
  const calendar = google.calendar({ version: 'v3', auth });
 ...
 */
export async function bootstrapWatchChannels(): Promise<void> {
