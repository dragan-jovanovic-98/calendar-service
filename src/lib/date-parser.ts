...
export interface ParseResult {
  success: boolean;
  slot?: ParsedTimeSlot;
  isRange?: boolean; // true for "after 4pm", "morning", etc.
  rangeStart?: Date; // for ranges, the earliest time
  rangeEnd?: Date; // for ranges, the latest time
  needsTimeSpecified?: boolean; // true when only a day was given (e.g., "Wednesday")
  error?: string;
}
 ...
 */
export function addMinutesToDate(date: Date, minutes: number, timezone: string): {
  date: Date;
  rfc3339: string;
} {
