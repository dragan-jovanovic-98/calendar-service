...
import { getWatchChannelByChannelId, updateWatchChannelSyncToken, clearWatchChannelSyncToken, getAppointmentByCalendarEventId, findLeadByEmail, createAppointmentFromGoogleBooking, getClientById, } from '../lib/supabase.js';
export async function googleWebhookRoutes(server) {
    /**
 ...
 */
async function processWebhookNotification(channelId) {
 ...
 */
async function processCalendarChanges(watchChannel, client) {
 ...
 */
async function processSingleEvent(event, clientId, client) {
    ...
    const externalAttendees = (event.attendees || []).filter((attendee) => attendee.email && !attendee.self);
    ...
    console.log(`Created Google booking appointment ${result.id} for client ${clientId}` +
...
//# sourceMappingURL=google-webhook.js.map